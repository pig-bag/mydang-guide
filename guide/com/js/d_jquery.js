...
function gnbBox_init(cnt){
	...
	$("#gnbBox > ul > li > a").hover(function(){gnbBox_ov(this)},function(){return false});
	...
	$("#gnbBox > ul > li > a").focus(function(){gnbBox_ov(this)});
	
}
function gnbBox_set(cnt){
	$("#gnbBox_ov").stop();
	cnt--;
	var point_href = $("#gnbBox > ul > li").eq(cnt).find(" > a").attr("href");
	var point_text = $("#gnbBox > ul > li").eq(cnt).find(" > a").text();
	var point_width = $("#gnbBox > ul > li").eq(cnt).width();
	var point_left = $("#gnbBox > ul > li").eq(cnt).position().left;
	
	if(navigator.appVersion.indexOf("MSIE 6")!=-1){
		//IE6.0용 코드
		var point_top = $("#gnbBox > ul > li").eq(cnt).position().top + $("#gnbBox > ul > li").height();	
	} else {	
		//그외 코드
		var point_top = $("#gnbBox > ul > li").eq(cnt).position().top;	
	}

	
	
	$("#gnbBox_ov").html('<strong><a href="'+point_href+'">'+point_text+'</a></strong>');
	$("#gnbBox_ov").width(point_width);
	if(gnbBox_animate == "Y"){
		$("#gnbBox_ov").animate(
			{top:point_top}
			,1
		...
		$("#gnbBox_ov").css({"left":point_left+"px","top":20+"px"});
	}
}
function gnbBox_ov(hit){
	var hitCount = $("#gnbBox > ul > li > a").index($(hit))+1;
	//실행
	gnbBox_set(hitCount);
}
...
function lnb_menu(lm1,lm2){
	//3차메뉴 기본 숨기기
	$("#lnb > li > ul").hide();
	$("#lnb > li > ul").animate({opacity:0});	
	for(i=0; i < $("#lnb > li").length; i++)
	{
		$("#lnb > li").eq(i).find(" > ul >li:last").css({"padding-bottom":"0px"});
	}
	//2차메뉴의 높이정의 및 기본높이저장
	//$("#lnb > li").css({"height":li_normal_h+"px","overflow":"hidden"});
	lm_height = $("#lnb > li").innerHeight();
	
	//2차메뉴 클릭시
	$("#lnb > li > a").click(function(){
		var lm_2th_check = $(this).parent().innerHeight();
	...
	$("#lnb > li > a").click(function(){lm_3th_show(this)});
	
	
	$("#lnb_con > a").click(function(){return false});
...
function lm_3th_show(data){	
	...
	$("#lnb > li > ul").hide();	
	$("#lnb > li > ul").animate({opacity:0});
	$("#lnb > li").animate({height:lm_height},lm_speed);
	
	$(data).parent().height($("#lnb > li > a").innerHeight());
	
	//3차메뉴 높이늘이기	
	if($(data).parent().find(">ul").length > 0){
		$(data).parent().animate(
			{
				height:$(data).parent().innerHeight()+$(data).parent().find(">ul").innerHeight()
			},
			lm_speed
		);
		$(data).parent().find(">ul").animate(
			{
				height:$(data).parent().find(" > ul").height()},lm_speed,
				function(){
					$(data).parent().find(" > ul").show();
					$("#lnb > li > ul").animate({opacity:1});
				}
		);
	} else {
		$(data).parent().animate(
			{
				height:$(data).parent().innerHeight()
			},
			lm_speed
		);
	}
...
function lm_open(){
	var li_count = $("#lnb > li").length;
	
	$("#lnb > li > ul").animate({opacity:1});	
	$("#lnb > li").height($("#lnb > li > a").innerHeight());
	for(i=0; i < li_count; i++)
	{		
		if($("#lnb > li").eq(i).find(">ul").length > 0)
		{
			$("#lnb > li").eq(i).animate(
				{
					height:$("#lnb > li").eq(i).innerHeight()+$("#lnb > li").eq(i).find(">ul").innerHeight()
				},
				lm_speed
			);
			$("#lnb > li").eq(i).find(">ul").animate(
				{
					height:$("#lnb > li").eq(i).find(">ul").innerHeight()},lm_speed,
					function(){
						$("#lnb > li > ul").show();					
					}
			);
			...
			$("#lnb > li").eq(i).animate(
				{
					height:$("#lnb > li").eq(i).innerHeight()
				},
				lm_speed
			);
		}
...
function lm_check(lm1,lm2){
	lm1--;
	lm2--;
	if($("#lnb > li").eq(lm1).find(">ul").length > 0){
		$("#lnb > li").eq(lm1).find(">ul").animate(
			{
				height:$("#lnb > li").eq(lm1).find(" > ul").height()},lm_speed,
				function(){
					$("#lnb > li").eq(lm1).find(" > ul").show();
					$("#lnb > li > ul").animate({opacity:1});
				}
		);
	}
	$("#lnb > li").eq(lm1).find(">a").toggleClass("link_1th_ov");
	$("#lnb > li").eq(lm1).find("ul>li").eq(lm2).find(">a").toggleClass("link_2th_ov");	
}
 ...
 * v 0.1.2 - 2007-08-13
 *	- fix/change: changed event binding routine, as is got really slow with $ 1.1.3.1
